...
instruction = ('Инструкция: Есть 2021 конфеты, вы берете их поочереди,\n'
    ...
    while candies_total > 0:
        if count == 0:
            ...
            if step > candies_total or step > max_take:
                step = int(input(
                    f'\nМожно взять только {max_take} конфет {lucky}, давай еще раз: '))
        ...
        if count == 1:
            ...
            if step > candies_total or step > max_take:
                step = int(input(
                    f'\nМожно взять только {max_take} конфет {loser}, давай еще раз: '))
            candies_total = candies_total - step
        if candies_total > 0:
            print(f'\nОсталось еще {candies_total}')
            count = 0
        else:
            print('Закончились конфеты')

    if count == 1:
        print(f'{loser} выиграл!')
    if count == 0:
        print(f'{lucky} выиграл!')
